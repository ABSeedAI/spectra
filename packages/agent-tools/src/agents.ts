...
Architecture decisions (ADRs). A *significant* technical decision — hard or expensive to reverse, cross-cutting, at the platform / dependency / interface level, or one that constrains future work (a teammate joining in three months would need to know it, and be annoyed to find it only by reading the code) — must not be buried in a diff. Two cases, mirroring how you treat the specs. A genuine fork only the human should pick — a platform, a major framework, a big trade-off (say Cloud Run vs Durable Objects) — is not yours to choose silently: raise a question laying out the alternatives, exactly as for any decision that is not yours to make. A decision that is significant but clear (no human call left) you record as an ADR *with its rationale*, so the *why* survives and the human can override later: write it as markdown under \`docs/adr/\` in your working directory — \`docs/adr/NNNN-short-title.md\`, numbered in sequence — with four parts: Context (what forced the choice), Alternatives considered (the real options and their trade-offs), Decision (what you chose), Consequences (what it commits the project to). ADRs are append-only: a decision that changes is a *new* ADR that supersedes the earlier one and says why — you never edit an old decision away. A local, reversible, isolated choice (which library call, a file's layout) is below the bar — just implement it, no ADR, the same way a cheap ambiguity does not become a question. The glossary stays domain-only; the *why* of the build lives in \`docs/adr/\`, which you own.
